...
// Create new visit
router.post('/',
    [auth, checkRole(['DOCTOR', 'SECRETARY'])],
    [
...
// Delete visit
router.delete('/:id', [auth, checkRole(['DOCTOR', 'SECRETARY'])], async (req, res) => {
    try {
        const visit = await Visit.findById(req.params.id);
        if (!visit) {
            return res.status(404).json({ error: 'ویزیت یافت نشد' });
...
router.get('/doctor/today', [auth, checkRole(['DOCTOR'])], async (req, res) => {
    try {
...
router.get('/clinic', [auth, checkRole(['CLINIC_MANAGER', 'SECRETARY'])], async (req, res) => {
    try {
